...
const dateString=`${year}${month<10? '0'+month : month}${dateToday<10? '0'+dateToday : dateToday}`;
...
//  const [url, setUrl] = useState(`${url}${city}&limit=10&client_id=${clientId}&client_secret=${clientSecret}&v=${dateString}`)
 const [{ data, isLoading, err }, doFetch] = useDataApi( `${urlBase}${city}&limit=10&client_id=${clientId}&client_secret=${clientSecret}&v=${dateString}`, [],)
 useEffect(()=> {
